...
AGENT_VERSION = "v8-gate"   # keep in sync with docs/SUBMISSIONS.md
...
OCCUPANCY_COST = 2.0      # water cost per occupied tile-day in the scorer;
                          # coupled to MIN_PLANT_SCORE, sweep the two together
...
# Lowered 16 -> 12 together with the harvest-day pricing above; the two only
# work as a pair. The old 16 was swept against a melon score of 65.7 that should
# have been ~15, so melon won every tile and the gate never bound. Priced
# correctly, melon stops taking tiles it cannot pay for -- and 16 admitted
# nothing to replace it (carrot 14.2, tomato 13.1, strawberry 6.4), so the farm
# just planted less. 12 admits exactly carrot and tomato. Measured over 240
# paired games on three disjoint seed sets: better in 141 (58.8%, p=0.0067),
# mean margin +$2,959, effect stable to within $330 across the three. A gate of
# 14 admits carrot but not tomato and is worth +$1,467 -- half, as predicted.
MIN_PLANT_SCORE = 12      # only plant a tile whose marginal score clears this.
    ...
    for _ in range(n_tiles):
        ...
        for crop, (units, occupancy, harvests) in proj.items():
            ...
            if start + units >= len(arr):
                continue
            score = (arr[start + units] - arr[start] - seed) / float(
                1 + OCCUPANCY_COST * occupancy + harvests)
...
def play(obs):
    ...
    target_hands = max(MIN_HANDS, min(MAX_HANDS, -(-work // ACTIONS_PER_UNIT) - 1))
