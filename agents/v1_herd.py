...
# Ladder evidence (5 games, 5 opponents): they ran 10.9-11.5 hands to our
# 12.4-12.8 and spent $5.5-7.8k on payroll to our $11.2-11.8k. The fib curve is
# convex -- 11 hands is $232/day, 12 is $376, 13 is $609 -- so the 13th hand has
# to earn ~$380/day, about eight watered tiles, while we idled ~1000 actions.
MAX_HANDS = 13
...
LAND_CASH_MULT = 1.8
MAX_QUADRANTS = 4         # the 4th costs $4,000 and no ladder opponent buys it
LAND_LATEST_DAY = 21      # past this a new quadrant cannot repay itself
SHED_PRESSURE = 78        # sell without regard to reserve above this shed level
HAUL_TRIGGER = 99999         # projected shed+carried at which units start hauling
...
def plan_land(obs, farm, orders, budget, used_tiles, owned_tiles):
    ...
    if owned - 1 >= len(LAND_PRICES) or len(orders) >= MAX_MARKET_ORDERS:
        return budget
    # The 4th quadrant costs $4,000 -- as much as the first two together -- and
    # is the corner furthest from the shed. Across five ladder games not one
    # opponent ever bought it, while we bought it every time and used only 3-15
    # of its 25 tiles. We are short of labour, not ground.
