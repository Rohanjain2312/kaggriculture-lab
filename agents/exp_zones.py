...
AGENT_VERSION = "v2-haul"   # keep in sync with docs/SUBMISSIONS.md
...
ZONE_PENALTY = 0          # priority charged for working outside your zone (0 = off)
PLANT_BLOCK = 0           # 1 = fill zones one at a time, 0 = nearest-to-shed first
...
def tile_task_cost(tile):
    """Rough daily unit-actions a tile demands, used to size zones."""
    if tile is None:
        return 1                      # a tile we intend to plant
    if not isinstance(tile, dict):
        return 0
    kind = tile.get("kind")
    if kind == "PLANT":
        return 2                      # water, plus a share of harvesting
    if kind == "WEED":
        return 1
    if tile.get("animal"):
        return 4                      # feed + care + collect + share of harvest
    return 1


def compute_zones(farm, board_size, n_units):
    """Split the worked tiles into `n_units` equal-cost wedges around the shed.

    For N tasks spread over area A the tour length runs as C*sqrt(N*A)
    (Beardwood-Halton-Hammersley; measured C ~1.2-1.7 here). Splitting the work
    among U units *without* splitting the ground leaves each unit crossing the
    whole farm, which costs sqrt(U) more travel -- at 13 units, 3.6x. Giving each
    unit its own patch keeps the total at C*sqrt(N*A) no matter how many units
    there are.

    Wedges radiate from the shed because every unit, farmer included, respawns
    there each morning, so the outbound leg is a daily cost paid per zone.
    """
    if n_units <= 1:
        return {}
    cx = cy = board_size / 2.0 - 0.5
    tiles = []
    for y in range(board_size):
        for x in range(board_size):
            tile = farm["tiles"][y][x]
            if tile == "LOCKED":
                continue
            cost = tile_task_cost(tile)
            if cost:
                tiles.append((math.atan2(y - cy, x - cx), x, y, cost))
    if not tiles:
        return {}
    tiles.sort()
    per = sum(t[3] for t in tiles) / float(n_units)
    zones = {}
    acc = 0.0
    z = 0
    for _, x, y, cost in tiles:
        zones[(x, y)] = min(z, n_units - 1)
        acc += cost
        if per > 0 and acc >= per * (z + 1):
            z += 1
    return zones


def build_jobs(obs, farm, private, board_size, seed_budget, plan, reserved,
               build_needs, zones=None):
    ...
    if plan and not final_day and obs["hour"] <= PLANT_LATEST_HOUR:
        if PLANT_BLOCK:
            # Fill one zone before opening the next, so a unit's crops sit in a
            # contiguous block. Costs longer walks early (the first block runs
            # outward) but shortens every watering sweep afterwards, and a spent
            # block converts to a ready-made contiguous wheat zone late on.
            blocks = zones or compute_zones(farm, board_size, PLANT_BLOCK)
            plantable.sort(key=lambda p: (blocks.get(p, 99),
                                          shed_distance(p, board_size), p))
        else:
...
def assign(jobs, units, unit_invs, seed_budget, board_size, zones=None):
    ...
    """
    shed = set(shed_tiles(board_size))
    # Match each unit to the zone it is actually nearest, rather than by index.
    # Handing unit i zone i sends units across the farm to reach "their" patch,
    # which costs more travel than having no zones at all.
    owner = {}
    if zones:
        centres = {}
        for (x, y), z in zones.items():
            cx, cy, n = centres.get(z, (0, 0, 0))
            centres[z] = (cx + x, cy + y, n + 1)
        centres = {z: (cx / n, cy / n) for z, (cx, cy, n) in centres.items()}
        free = set(centres)
        for i, upos in enumerate(units):
            if not free:
                break
            best = min(free, key=lambda z: (abs(centres[z][0] - upos[0])
                                            + abs(centres[z][1] - upos[1]), z))
            owner[best] = i
            free.discard(best)
        owner = {z: u for z, u in owner.items()}
    zone_of_unit = {u: z for z, u in owner.items()}
    ...
    for j, (priority, pos, action, req) in enumerate(jobs):
        for i, upos in enumerate(units):
            if req is not None and unit_invs[i].get(req, 0) <= 0:
                continue
            d = dist(upos, pos)
            score = priority - MOVE_PENALTY * d
            # Zones are a preference, not a fence. Work that dies tonight, and
            # anything at the shed, stays open to whoever is nearest -- a strict
            # partition would let a plant weed while an idle unit sat one zone
            # over, which is the failure the priority system exists to prevent.
            if zones and priority < P_WATER_CRIT and pos not in shed:
                if zone_of_unit.get(i) != zones.get(pos, zone_of_unit.get(i)):
                    score -= ZONE_PENALTY
...
def play(obs):
    ...
    zones = compute_zones(farm, board_size, units_count) if ZONE_PENALTY else None
    jobs, plantable = build_jobs(obs, farm, private, board_size, seed_budget,
                                 plan, reserved, build_needs, zones)
    actions = assign(jobs, units, unit_invs, seed_budget, board_size, zones)
